...
# Create the given Elasticsearch ILM policy.
function create_ilm_policy {
	local name=$1
	local json=$2

	local -a args=( '-s' '-D-' '-m15' '-w' '%{http_code}'
		"http://${ELASTIC_HOST}:${ELASTIC_PORT}/_ilm/policy/${name}"
		'-X' 'PUT'
		'-H' 'Content-Type: application/json'
		'-d' "$json"
		)
...
# Create the given Elasticsearch index template.
function create_index_template {
	local name=$1
	local json=$2

	local -a args=( '-s' '-D-' '-m15' '-w' '%{http_code}'
		"http://${ELASTIC_HOST}:${ELASTIC_PORT}/_index_template/${name}"
		'-X' 'PUT'
		'-H' 'Content-Type: application/json'
		'-d' "$json"
		)
...
function create_kibana_objects {
	...
	local -a args=( '-s' '-D-' '-m15' '-w' '%{http_code}'
		"http://${KIBANA_HOST}:${KIBANA_PORT}/api/saved_objects/_import"
		'-X' 'POST'
		'-H' 'kbn-xsrf: true'
		'--form' "file=@$path_ndjson"
		)
